 ...
 * @param   {Request} req
 */
async function getRoutes(req = null) {
    let shopID =  req.headers['shop-id'] ;
    if(!shopID){
      return {} ;
    }
  return new Promise((resolve, reject) => {

    fetch("https://babshaye.com/api/v1/2/sitemap-data").then(function (response) {
 ...
 * @param productRoutes
 */
function formatShopRoutes(productRoutes) {
  let routes = [{url: hostname()}];
  if(!Object.keys(productRoutes).length)  return routes ;

  productRoutes.brand.forEach(item => {
    routes.push({url: hostname() + '/' + item});
  });

  productRoutes.category.forEach(item => {
    routes.push({url: hostname() + '/' + item});
  });

  productRoutes.product.forEach(item => {
    routes.push({url: hostname() + '/' + item});
  });
...
module.exports = { createSitemap, createSitemapIndex , getRoutes, formatShopRoutes}
